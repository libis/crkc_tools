...
//***************************************
//**** KUNSTVOORWERP_HAS_MATERIAAL  *****
//***************************************

print "IMPORTING kunstvoorwerp has materiaal \n";
...
while($o_tab_parser->nextRow()) 
{
	...
	//Term opzoeken in move_materiaal
	$vn_Term_waarde = $t_list->getItemIDFromListByLabel('move_materiaal', trim($va_Term_waarde[0]));
	print " waarde_id {$vn_Term_waarde} \n ";

	if ($vn_Term_waarde) {
		
		print "record werd reeds verwerkt \n";
		

/*		
		Print " success: materiaal gevonden: {$vn_Term_id} / {$va_Term_waarde[0]} / {$va_Term_waarde[1]} / {$vn_Term_waarde} \n";
		...
		if ($dimensions > 0) {
			...
			if ($t_object->numErrors()) {
				...
				$t_object->addAttribute(array(
					...
					'objectMateriaalType'		=>	$vn_Term_waarde,
					'materiaalOpmerking'		=>	$vs_Commentaar
				), 'objectMateriaalInfo');
...
*/
		...
		print " materiaal {$va_Term_waarde[0]} voor {$vn_Kunstvoorwerp_id} niet gevonden in move_materiaal \n";
		print " plaatsen gezochte tekst in opmerkingen veld \n";
		$vn_Term_waarde = $t_list->getItemIDFromList('move_materiaal', 'blank');
		print " waarde_id {$vn_Term_waarde} \n ";
...
print "END IMPORTING kunstvoorwerp_has_materiaal \n";
