...
print "relatie object-vervaardiger: ".$vn_objects_x_entities_id."\n";
...
print "IMPORTING new_kunstenaar into an array \n";
...
while($o_tab_parser->nextRow()) {
	...
	$vn_Kunstenaar_id		=	$o_tab_parser->getRowValue(1); //id kunstvoorwerp
	...
	$va_vervaardiger[$vn_Kunstenaar_id] = "verv_" . $vn_Kunstenaar_id;
...
if (!$o_tab_parser->parse('/www/libis/html/ca_crkc/app/lib/ca/data/new_kunstvoorwerp_has_kunstenaar.csv')) {
	die("Couldn't parse dml_kunstvoorwerp_has_kunstenaar.csv data\n");	
}
...
while($o_tab_parser->nextRow()) {
	...
	$vn_Kunstenaar_id		=	$o_tab_parser->getRowValue(2);
	...
	$vs_Kunstenaar_id = "verv_".$vn_Kunstenaar_id;	
	...
	if ($dimensions > 0) {
		...
		$va_right_id = $t_entities->getEntityIDsByidno($vs_Kunstenaar_id);
		
		print "right_id_1:".$va_right_id[0]."\n";
		
		if ($va_right_id == NULL) {
			print " ERROR: VERVAARDIGER {$vs_Kunstenaar_id} toevoegen \n";
		}else{
			$t_object -> addRelationship('ca_entities', $va_right_id[0], $vn_objects_x_entities_id);
			print "done\n";
			if ($t_object->numErrors()) {
				print "ERROR LINKING object and vervaardiger : ".join('; ', $t_object->getErrors())."\n";
				continue;
			} else {
				print "link object-vervaardiger succesvol\n";
			}
/*			
			if (strlen(trim($vs_opmerking)) > 0) {
				$t_object->addAttribute(array(
					'locale_id'				=>	$pn_locale_id,
					'vervaardigerOpmerkingen'		=>	$vs_opmerking
