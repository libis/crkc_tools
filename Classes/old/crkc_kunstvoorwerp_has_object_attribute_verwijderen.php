...
print "READING new_kunstvoorwerp_has_***.csv...\n";
...
//	print "removing attributes from ".$vn_Kunstvoorwerp_id."/".$vs_crkcObjectnr."  \n";
...
//----------------
	...
	print "aantal objecten: {$dimensions}\n";
...
//-------------------------
		...
		$aantal = $t_object->getAttributeCountByElement('anderTrefwoordList');
		
		print "aantal attributen: {$aantal}\n";
	
		$va_attrs = $t_object->getAttributesByElement('anderTrefwoordList');
		...
		while($aantal_attrs > 0) 
		{
			...
			if ($t_object->numErrors()) {
				print "ERROR REMOVING ATTIBUTE anderTrefwoordList ".$vn_Kunstvoorwerp_id.": ".join('; ', $t_object->getErrors())."\n";
			} else {
