<?php
// Opgelet de configuratie moet juist geplaatst worden zodat het europees formaat geladen wordt. Het eenvoudigste is als je core/Parsers/TimeExpressionParser/en_US.lang aanpast naar europese datum. Je past ook best de locale aan, maar dat werkt niet altijd.
// opgelet sommige datums bevatten 00 als dag. Het conversie script voor bruikleen is aangepast. Het zet de dag 00 om naar 01
/*
 * Step 1: Initialisation
 */
 
set_time_limit(36000);
require_once("/www/libis/html/ca_crkc/setup.php");

require_once(__CA_LIB_DIR__.'/core/Db.php');
require_once(__CA_MODELS_DIR__.'/ca_locales.php');
require_once(__CA_MODELS_DIR__.'/ca_loans.php');

require_once(__CA_LIB_DIR__.'/core/Parsers/DelimitedDataParser.php');

//require_once(__CA_LIB_DIR__.'/core/Parsers/TimeExpressionParser.php');
//$o_date_parser = new TimeExpressionParser();
//$o_date_parser->setLanguage('nl_NL',__PHPWEBLIB_DIR__);
//$vs_Datum_bis	=	$o_date_parser->preprocess($vs_Datum);

$_ = new Zend_Translate('gettext', __CA_APP_DIR__.'/locale/en_US/messages.mo', 'nl_NL');

$t_locale = new ca_locales();
$pn_locale_id = $t_locale->loadLocaleByCode('nl_NL');

$t_list = new ca_lists();
$pn_loans_type_id =	$t_list->getItemIDFromList('loan_types', 'tijdelijkeBruikleen');	

//************************************
//****     BRUIKLEEN     *****
//************************************

print "IMPORTING Bruikleen\n";

/*
// * Step 2: Import
*/

 // want to parse comma delimited data? Pass a comma here instead of a tab.
$o_tab_parser = new DelimitedDataParser("\t"); 

// Read csv; line by line till end of file.
if (!$o_tab_parser->parse('/www/libis/html/ca_crkc/app/lib/ca/data/new_bruikleen.csv')) {
	die("Couldn't parse new_bruikleen.csv data\n");	
}
	
print "READING new_bruikleen.csv...\n";

$vn_c = 1;
	
$o_tab_parser->nextRow(); // skip first row
//-------------------------
// waarden inlezen
//-------------------------
while($o_tab_parser->nextRow()) {
	// Get columns from tab file and put them into named variables - makes code easier to read
	$vn_Bruikleen_id	=	$o_tab_parser->getRowValue(1);
	$vs_Identificatie	=	$o_tab_parser->getRowValue(3);
	$vs_Omschrijving	=	$o_tab_parser->getRowvalue(4);
	$vs_Plaats		=	$o_tab_parser->getRowvalue(5);
	$vs_Begindatum		=	$o_tab_parser->getRowvalue(6);
	$vs_Einddatum		=	$o_tab_parser->getRowvalue(7);
	//$vs_Origine		=	$o_tab_parser->getRowvalue(11);
	
	// opkuisen van begin en einddatum
	if(strstr($vs_Begindatum,'00/'))
	{
		$vs_Begindatum = str_replace("00/", "01/",$vs_Begindatum);
	} 
	if(strstr($vs_Einddatum,'00/')){
		$vs_Einddatum = str_replace("00/", "01/",$vs_Einddatum);
	}
	
print "\n Creating  ".$vn_Bruikleen_id.".".$vs_Identificatie." and adding labels for Bruikleen. \n"; 
 
// ------------------------------------
// Waarden omvormen naar gepaste output
// ------------------------------------

//Datumveld: Begindatum - Einddatum

if (trim($vs_Begindatum) == "" and trim($vs_Einddatum) == "") {
	$vs_Datum = "";
} elseif (trim($vs_Begindatum) == "" and trim($vs_Einddatum) <> "") {
	$vs_Datum = $vs_Einddatum;	
} elseif (trim($vs_Begindatum) <> "" and trim($vs_Einddatum) == "") {
	$vs_Datum = $vs_Begindatum;
} else {
	$vs_Datum = $vs_Begindatum." - ".$vs_Einddatum;
}

print "Datum : ".$vs_Datum."\n";

/*if (trim($vs_Origine) == 'CRKC') {
	$pn_occurrence_source_id =	$t_list->getItemIDFromList('occurrence_sources', 'os_crkc');
} elseif (trim($vs_Origine) == 'POV') {
	$pn_occurrence_source_id =	$t_list->getItemIDFromList('occurrence_sources', 'os_pov');
}elseif (trim($vs_Origine) == 'PA') {
	$pn_occurrence_source_id =	$t_list->getItemIDFromList('occurrence_sources', 'os_pa');
} else {
	$pn_occurrence_source_id =	null;
	print "   - WARNING: geen Bron opgegeven voor {$vs_Identificatie}, verbeter indien nodig \n";
}*/


//----------------		
// Create object
//----------------

	$t_loan = new ca_loans();
	$t_loan->setMode(ACCESS_WRITE);
	$t_loan->set('type_id', $pn_loans_type_id);  	//bruikleen
	//$t_loan->set('source_id', $pn_occurrence_source_id); Er is geen source voor loans
	$t_loan->set('idno', "bruikl_".$vn_Bruikleen_id);
	$t_loan->set('access', 1);
	$t_loan->set('status', 4);
//-------------------------
// Waarden mappen      
//-------------------------

	$t_loan->addAttribute(array(
		'locale_id'	=>	$pn_locale_id,
		'name'		=>	$vs_Identificatie
	), 'name');

	if (trim($vs_Omschrijving) != '') {
		$t_loan->addAttribute(array(
		'locale_id'			=>	$pn_locale_id,
		'bruikleenReden'	=>	$vs_Omschrijving
	), 'bruikleenReden');

	}

	if (trim($vs_Plaats) != '') {
		$t_loan->addAttribute(array(
		'locale_id'		 	=>	$pn_locale_id,
		'bruikleenPlaats'		=>	$vs_Plaats
	), 'bruikleenPlaats');

	}

	if (trim($vs_Datum)!= '') {
		$t_loan->addAttribute(array(
		'locale_id'		=>	$pn_locale_id,
		'tijdelijkBruikleenDatum'	=>	$vs_Datum
	), 'tijdelijkBruikleenDatum');

	}

	// insert the object
	$t_loan->insert();

	if ($t_loan->numErrors()) {
		print "\tERROR INSERTING {$vn_Bruikleen_id}/{$vs_Identificatie}: ".join('; ', $t_loan->getErrors())."\n";
		continue;
	}


	// --------------------------------------------------------------
	// Set a preferred label for the object
	// --------------------------------------------------------------
		
	// DON'T FORGET TO GIVE EVERYTHING A LABEL! This is the value used for display in search results
	// and lots of other places. If you don't define a value it will be hard to distinguish this row
	// for others


	$t_loan->addLabel(
		array('name' => $vs_Identificatie),
		$pn_locale_id, null, true
	);
		
	if ($t_loan->numErrors()) {
		print "\tERROR ADD LABEL TO {$vn_Bruikleen_id}/{$vs_Identificatie}: ".join('; ', $t_loan->getErrors())."\n";
		continue;
	}

/*	$lengte = 0;
	$beginpos = 0;
	unset($vs_Vervaardiger);
	unset($vn_Vervaardiger_id);
	$vs_AATtermnummer_bis = "";
*/

	$vn_c++;
}

print "ENDED IMPORTING new_tentoonstelling.csv\n";

?>
