<?php
	...
	$vn_objects_x_occurrences_id = $t_rel_types->getRelationshipTypeID('ca_objects_x_occurrences', 'used'); // werd gebruikt in
	print "relatie object-overigeevenementen: ".$vn_objects_x_occurrences_id."\n";
...
if (!$o_tab_parser->parse('/www/libis/html/ca_crkc/app/lib/ca/data/new_kunstvoorwerp_has_overigeevenementen.csv')) {
	die("Couldn't parse new_kunstvoorwerp_has_overigeevenementen.csv data\n");	
}
...
while($o_tab_parser->nextRow()) {
	...
	$vn_Evenement_id		=	$o_tab_parser->getRowValue(2);
	$vs_Commentaar 			= 	$o_tab_parser->getRowValue(3);
		
	print "PROCESSING {$vn_Kunstvoorwerp_id}\n";

	$vs_Evenement_id = "overig_".$vn_Evenement_id;
...
//----------------
	...
	if ($dimensions > 0) {
		...
		$va_right_id = $t_occurrences->getOccurrenceIDsByidno($vs_Evenement_id);
		print "right_id_1:".$va_right_id[0]."\n";
		
		if ($va_right_id == NULL) {
			print " ERROR: overige evenement {$vs_Evenement_id} toevoegen \n";
		}else{
			$t_object -> addRelationship('ca_occurrences', $va_right_id[0], $vn_objects_x_occurrences_id);
			print "done\n";
			if ($t_object->numErrors()) {
				print "ERROR LINKING object and overige evenement : ".join('; ', $t_object->getErrors())."\n";
				continue;
			} else {
				print "link object-overigeevenement succesvol\n";
			}
			
			if(trim($vs_Commentaar) != "")
			{
				$t_object->addAttribute(array(
					'locale_id'			=>	$pn_locale_id,
					'evenementOpmerking'		=>	$vs_Commentaar
				), 'evenementOpmerking');
			}
