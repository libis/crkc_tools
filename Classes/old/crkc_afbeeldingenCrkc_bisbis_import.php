...
while($o_tab_parser->nextRow()) {
	...
	if (substr(trim($vs_crkcObjectnr),0,4) == "CRKC" )
	{
...
// Read csv; line by line till end of file.
if (!$o_tab_parserAfbeeldingen->parse('/www/libis/html/ca_crkc/app/lib/ca/data/afbeeldingenCrkc2.csv')) {
	die("Couldn't parse afbeeldingenCrkc.csv data\n");	
}
	
print "READING afbeeldingenCrkc.csv...\n";
...
*/
			...
			if (trim($pid)) {
				...
				$t_attrib->addAttribute(57,$object_ids[0],'digitoolUrl', array(
					'locale_id'	=>	$pn_locale_id,
					'digitoolUrl'	=>	trim($pid)
				));
			...
			if ($t_attrib->numErrors()) {
				print "\tERROR INSERTING {$object_ids} / {$label} : ".join('; ', $t_attrib->getErrors())."\n";
				continue;
			} else {
