...
// Read csv; line by line till end of file.
if (!$o_tab_parserAfbeeldingen->parse('/www/libis/html/ca_crkc/app/lib/ca/data/afbeeldingenPov.csv')) {
	die("Couldn't parse afbeeldingenPov.csv data\n");	
}
	
print "READING afbeeldingenPov.csv...\n";
...
print "\n onderzoeken array met {$vn_c2} afbeeldingen. \n"; 
...
foreach($afbeeldingen as $label_key => $pid_value)
{
	...
	{
		print "array_key exists /idno: {$pid_value} \n";
		...
		{	
			...
			if ($t_object->numErrors()) {
				print "\tERROR INSERTING {$object_ids}/{$pid_value}: ".join('; ', $t_object->getErrors())."\n";
				continue;
			} else {
				print "toevoegen van object: " . $label_key . " gelukt \n";
			}				
		} else {
		...
		print "niks gevonden" . $lookup . " idno: " . $kunstvoorwerpIdnos[$lookup] ."\n" ;	
	}

	
	print "===={$teller} / {$succes} =====================Volgende afbeelding======================\n";	
