...
*******************************
*/
print "IMPORTING STIJL_LIJST\n";

$t_list->load(array('list_code' => 'stijl_lijst')); 

if ($t_item = $t_list->addItem('Westerse stijl', true, false, null, null, 'WesterseStijl','', 4, 1, null)) {
	// add preferred labels
	if (!($t_item->addLabel(
		array('name_singular' => 'Westerse stijl', 
			'name_plural' => 'Westerse stijl', 
			'description' => ''),
		$pn_locale_id, null, true
	))) {
		print "ERROR: Could not add preferred label to stijlen_lijst Westerse stijl : ".join("; ", $t_item->getErrors())."\n";
	}
}

if ($t_item = $t_list->addItem('Etnografische stijl', true, false, null, null, 'EtnografischeStijl ','', 4, 1, null)) {
	// add preferred labels
	if (!($t_item->addLabel(
		array('name_singular' => 'Etnografische stijl', 
			'name_plural' => 'Etnografische stijl', 
			'description' => ''),
		$pn_locale_id, null, true
	))) {
		print "ERROR: Could not add preferred label to stijlen_lijst Etnografische stijl : ".join("; ", $t_item->getErrors())."\n";
	}
}
	
if ($t_item = $t_list->addItem('Orientaalse stijl', true, false, null, null, 'OrientaalseStijl','', 4, 1, null)) {
	// add preferred labels
	if (!($t_item->addLabel(
		array('name_singular' => 'Orientaalse stijl', 
			'name_plural' => 'Orientaalse stijl', 
			'description' => ''),
		$pn_locale_id, null, true
	))) {
		print "ERROR: Could not add preferred label to stijlen_lijst Orientaalse stijl : ".join("; ", $t_item->getErrors())."\n";
	}
}
...
if ($t_item = $t_list->addItem($vs_stijlen_naam, true, false, $vn_Parent, null, $vs_AATtermnummer,'', 4, 1, null)) {
	// add preferred labels
	if (!($t_item->addLabel(
		...
		print "ERROR: Could not add preferred label to stijlen_lijst ".$vs_stijlen_naam.": ".join("; ", $t_item->getErrors())."\n";
	}
}					
$vn_c++;
}
